"""
db/pdf_factura.py
Genera un PDF con el detalle de una factura (encabezado de la empresa,
...
def _carpeta_pdfs():
    ...
    carpeta = os.path.join(base, "facturas_pdf")
    os.makedirs(carpeta, exist_ok=True)
    return carpeta


def generar(factura_id):
    """
    Genera el PDF de una factura y devuelve {"ok": True, "ruta": "..."}
    o {"ok": False, "error": "..."} si algo sale mal.
    """
    fac = facturas_db.obtener_con_detalle(factura_id)
    if not fac:
        return {"ok": False, "error": "La factura ya no existe."}
    ...
    ruta = os.path.join(_carpeta_pdfs(), f"{fac['numero']}.pdf")
    ...
    encabezado = Table(
        [[Paragraph(nombre_empresa, titulo), Paragraph(f"FACTURA<br/>{fac['numero']}", numero_style)]],
    ...
    info = Table(
        [
            ["Cliente:", fac.get("cliente_nombre", "Consumidor final")],
            ["Fecha:", str(fac.get("fecha", ""))[:10]],
            ["Método de pago:", fac.get("metodo_pago", "—")],
            ["Estado:", fac.get("estado", "Pagada")],
        ],
    ...
    if fac.get("notas"):
        ...
        elementos.append(Paragraph(fac["notas"], normal))
